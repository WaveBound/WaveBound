...
<div class="upgrade-container" style="max-width: 900px; flex-direction: column;">
    ...
    <div style="display: flex; align-items: center; margin-top: 10px;">
        ...
        <input type="text" id="default-upgrade-text" style="
            height: 35px;
            font-size: 16px;
            font-weight: bold;
            width: 150px;
...
function toggleSettingsDropdown(event) {
    ...
    // Check if this is the anti-afk dropdown
    const isAntiAfk = dropdownContainer.closest('.settings-container.sub[style*="margin-top: 20px"]');
    
    const buttonRect = dropdownContainer.getBoundingClientRect();
    const dropdownHeight = isAntiAfk ? 170 : 204; // Different height for anti-afk dropdown
    ...
    // Position anti-afk dropdown right above the button
    if (isAntiAfk) {
...
# In the handle_wave function, update the upgrade queueing section:
        for upgrade in upgrades_data:
            try:
                ...
                # Add check for unit placement before queueing upgrade
    ...
    def get_click_location(self, element_id, is_unit_card=False, location_number=None):
        def handle_click(coords):
            if 'anti-afk-click-status' in element_id:
                location_num = element_id.split('-')[-1]  # Extract number from element ID
                print(f"Processing Anti-AFK Location {location_num} at coordinates X={coords[0]}, Y={coords[1]}")
                
                # Save coordinates to antiAfkClickLocations
                js_save_coords = f"""
                    if (!window.antiAfkClickLocations) {{
                        window.antiAfkClickLocations = {{}};
                    }}
