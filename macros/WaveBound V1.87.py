...
function toggleDropdown(event, options = {}) {
    ...
    const openUpward = spaceBelow < customHeight;
    
    dropdownManager.toggle(event, {
        customWidth: customWidth,
        customHeight: customHeight,
        positionAbove: openUpward,
        isAntiAfk: isAntiAfk,
        onSelect: onSelect
    });
}
...
// For ability dropdown
function toggleAbilityDropdown(event) {
    toggleDropdown(event, { customHeight: 204 });
}

// For unit dropdown
function toggleUnitDropdown(event) {
    toggleDropdown(event, { customWidth: 135, customHeight: 225 });
}
...
class Api:
    ...
    def upgrade_macro(self, unit_number, upgrade_text, upgrade_number):
        ...
        while True:
            ...
            ability_queue_items = list(self.ability_queue.queue)
            ability_timer_items = list(self.ability_timers)
            current_time = time.time()
    
            if ability_queue_items or ability_timer_items:
                print(f"Found {len(ability_queue_items)} abilities in queue during upgrade")
                print(f"Found {len(ability_timer_items)} abilities in timer list")
                
                # Process timer list first - these need to be executed
                for timer, unit, number in ability_timer_items:
                    if current_time >= timer and number not in self.completed_abilities:
                        # Update interruption text before executing ability
                        all_text, _ = self.search_text("", self.get_upgrade_region(), return_all_text=True)
                        if all_text and len(all_text) > 0:
                            interruption_text = all_text[0]
                            check_text = interruption_text
                            print(f"Updated interruption text before executing ability: {interruption_text}")
                        
                        print(f"Executing overdue ability {number} for unit {unit}")
                        self.activate_ability(unit, number)
                        self.ability_timers.remove((timer, unit, number))
                        found_initial_text = False
                        menu_open = False
                ...
                # Then process queue - execute if delay is 0 or negative
                while not self.ability_queue.empty():
                    ability_unit, ability_delay, ability_number = self.ability_queue.get()
                    if ability_number not in self.completed_abilities:
                        if ability_delay <= 0:
                            ...
                            print(f"Executing immediate/overdue ability {ability_number} for unit {ability_unit}")
                            self.activate_ability(ability_unit, ability_number)
                            found_initial_text = False
                            menu_open = False
                        else:
                            # Just add to timer list without resetting menu state
                            start_time = time.time()
                            timer_entry = (start_time + ability_delay, ability_unit, ability_number)
                            if timer_entry not in self.ability_timers:
                                self.ability_timers.append(timer_entry)
                                print(f"Queued ability {ability_number} with delay {ability_delay}")
                        continue
            ...
            if current_wave and current_wave not in self.handled_waves:
                # Update interruption text before handling wave
                all_text, _ = self.search_text("", self.get_upgrade_region(), return_all_text=True)
                if all_text and len(all_text) > 0:
                    interruption_text = all_text[0]
                    check_text = interruption_text
                    print(f"Updated interruption text before handling wave: {interruption_text}")
            ...
            # Upgrade phase
                ...
                if "Not Set" not in upgrade_click_status:
                    ...
                    # Get latest interruption text before checking abilities
                    all_text, _ = self.search_text("", self.get_upgrade_region(), return_all_text=True)
                    if all_text and len(all_text) > 0:
                        interruption_text = all_text[0]
                        print(f"Updated interruption text to: {interruption_text}")
