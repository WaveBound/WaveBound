            ...
            <div style="padding: 50px 15px 15px 15px; display: flex; flex-direction: column; align-items: center; flex: 1; overflow: hidden;">
                <div style="display: flex; flex-direction: column; gap: 8px; align-items: center; width: 100%;">
                    ...
                    <label class="checkbox-label" style="transform: scale(1.2);">
                        <input type="checkbox" class="checkbox" id="onReplayCheck" style="width: 24px; height: 24px;" onclick="handleCheckboxClick('onReplayCheck', 'onStartCheck')">
                        <span style="font-size: 18px;">On Replay</span>
                    </label>

                    <label class="checkbox-label" style="transform: scale(1.2);">
                        <input type="checkbox" class="checkbox" id="onStartCheck" style="width: 24px; height: 24px;" onclick="handleCheckboxClick('onStartCheck', 'onReplayCheck')">
            ...
            if (!isVisible) {
                ...
                if (!existingMenu.children.length) {
                    items.forEach(item => {
                        ...
                        a.href = '#';
                        a.style.cssText = 'color: #c9d1d9; display: block; padding: 8px 10px; text-decoration: none; text-align: left;';
                        a.textContent = item;
                ...
                setTimeout(() => {
                    window.addEventListener('click', (e) => {
                        if (!button.contains(e.target) && !existingMenu.contains(e.target)) {
                            existingMenu.style.display = 'none';
                            this.activeDropdown = null;
                        }
                    }, { once: true });
...
function drag(ev) {
    const isNewItem = ev.target.closest('#actionsList') === null;
    const dragText = ev.target.innerText;
    const formattedText = getDisplayText(dragText, {
        x: '0', y: '0',
    ...
    ev.target.dataset.dragText = formattedText;
    ev.dataTransfer.setData("text", dragText);
    ev.dataTransfer.setData("isNewItem", isNewItem);
}
...
function getDisplayText(type, settings) {
    switch(type) {
        case 'Move And Click':
...
function updateActionText(actionItem) {
    const actionText = actionItem.querySelector('span');
    actionText.textContent = getDisplayText(actionItem.dataset.type, actionItem.settings);
}

function addDropPreview() {
    const actionsList = document.getElementById('actionsList');
    let previewItem = null;
    
    actionsList.addEventListener('dragover', (e) => {
        e.preventDefault();
        const dropTarget = e.target.closest('#actionsList > div');
        ...
        if (!draggingItem && !previewItem) {
            previewItem = document.createElement('div');
            previewItem.className = 'dragging';
            ...
            previewItem.innerHTML = `
                <span style="color: #c9d1d9; font-size: 16px;">${dragText}</span>
                <button style="background: none; border: none; color: #c9d1d9; font-size: 16px; padding: 0 5px;">✕</button>
            `;
            actionsList.appendChild(previewItem);
        }
        
        const itemToMove = draggingItem || previewItem;
        if (!itemToMove) return;

        const closestItem = Array.from(actionsList.children)
            .filter(child => child !== itemToMove)
        ...
        if (closestItem) {
            ...
            actionsList.insertBefore(itemToMove, isAfter ? closestItem.element.nextSibling : closestItem.element);
        }
    });
    ...
    actionsList.addEventListener('drop', () => {
        previewItem?.remove();
        previewItem = null;
    });
...
// Call this function to initialize the preview
addDropPreview();

function drop(ev) {
    ev.preventDefault();
    const actionsList = document.getElementById('actionsList');
    const isNewItem = ev.dataTransfer.getData("isNewItem") === "true";
    const data = ev.dataTransfer.getData("text");
    
    if (isNewItem) {
        const actionItem = document.createElement('div');
        actionItem.dataset.type = data;
        actionItem.settings = {
            x: '0', y: '0',
            amount: '0', direction: 'up',
            duration: '0',
        ...
        actionItem.ondblclick = function() {
            if (actionItem.isBeingDeleted) return;
...
case 'Move And Click':
    ...
    xLabel.style.cssText = 'color: #c9d1d9; width: 20px;';

    const xInput = document.createElement('input');
    xInput.type = 'text';
    xInput.id = 'click-x';
    xInput.value = actionItem.settings.x || '0';
    xInput.style.cssText = 'width: 100px; height: 30px; background-color: #0d1117; color: #c9d1d9; border: 1px solid #30363d; border-radius: 3px; padding: 0 10px;';

    const yLabel = document.createElement('span');
    yLabel.textContent = 'Y:';
    yLabel.style.cssText = 'color: #c9d1d9; width: 20px;';

    const yInput = document.createElement('input');
    yInput.type = 'text';
    yInput.id = 'click-y';
    yInput.value = actionItem.settings.y || '0';
    yInput.style.cssText = 'width: 100px; height: 30px; background-color: #0d1117; color: #c9d1d9; border: 1px solid #30363d; border-radius: 3px; padding: 0 10px;';

    const delayLabel = document.createElement('span');
    delayLabel.textContent = 'Delay:';
    delayLabel.style.cssText = 'color: #c9d1d9; width: 50px;';

    const delayInput = document.createElement('input');
    delayInput.type = 'text';
    delayInput.id = 'click-delay';
    delayInput.value = actionItem.settings.delay || '0';
    delayInput.style.cssText = 'width: 100px; height: 30px; background-color: #0d1117; color: #c9d1d9; border: 1px solid #30363d; border-radius: 3px; padding: 0 10px;';
    ...
    coordContainer.appendChild(xLabel);
    coordContainer.appendChild(xInput);
    coordContainer.appendChild(yLabel);
    coordContainer.appendChild(yInput);
    coordContainer.appendChild(delayLabel);
    coordContainer.appendChild(delayInput);

    content.appendChild(coordContainer);
    content.appendChild(setLocationBtn);
    break;

case 'Custom OCR':
    const ocrContainer = document.createElement('div');
    ocrContainer.style.cssText = 'display: flex; flex-direction: column; gap: 15px;';
    
    const targetTextContainer = document.createElement('div');
    targetTextContainer.style.cssText = 'display: flex; align-items: center; gap: 10px;';
    
    const targetLabel = document.createElement('span');
    targetLabel.textContent = 'Target Text:';
    targetLabel.style.cssText = 'color: #c9d1d9;';
    
    const targetInput = document.createElement('input');
    targetInput.type = 'text';
    targetInput.id = 'ocr-text';
    targetInput.value = actionItem.settings.text || '';
    targetInput.style.cssText = 'width: 200px; height: 30px; background-color: #0d1117; color: #c9d1d9; border: 1px solid #30363d; border-radius: 3px; padding: 0 10px;';
    
    const regionContainer = document.createElement('div');
    regionContainer.style.cssText = 'display: flex; flex-direction: column; gap: 10px;';
    
    const coordsContainer = document.createElement('div');
    coordsContainer.style.cssText = 'display: flex; align-items: center; gap: 10px;';
    
    const coordInputs = ['x1', 'y1', 'x2', 'y2'].map(coord => {
        const container = document.createElement('div');
        container.style.cssText = 'display: flex; align-items: center; gap: 5px;';
        
        const label = document.createElement('span');
        label.textContent = coord.toUpperCase() + ':';
        label.style.cssText = 'color: #c9d1d9; width: 25px;';
        
        const input = document.createElement('input');
        input.type = 'text';
        input.id = `ocr-${coord}`;
        input.value = actionItem.settings[coord] || '0';
        input.style.cssText = 'width: 60px; height: 30px; background-color: #0d1117; color: #c9d1d9; border: 1px solid #30363d; border-radius: 3px; padding: 0 10px;';
    ...
    targetTextContainer.appendChild(targetLabel);
    targetTextContainer.appendChild(targetInput);
    
    coordsContainer.append(...coordInputs);
    
    regionContainer.appendChild(coordsContainer);
    regionContainer.appendChild(setRegionBtn);
    
    ocrContainer.appendChild(targetTextContainer);
    ocrContainer.appendChild(regionContainer);
    
    content.appendChild(ocrContainer);
    break;

case 'Scroll':
    const scrollContainer = document.createElement('div');
    scrollContainer.style.cssText = 'display: flex; flex-direction: column; gap: 10px;';
    
    const scrollAmountContainer = document.createElement('div');
    scrollAmountContainer.style.cssText = 'display: flex; align-items: center; gap: 10px;';
    
    const scrollLabel = document.createElement('span');
    scrollLabel.textContent = 'Scroll Amount:';
    scrollLabel.style.cssText = 'color: #c9d1d9;';
    
    const scrollInput = document.createElement('input');
    scrollInput.type = 'text';
    scrollInput.id = 'scroll-amount';
    scrollInput.value = actionItem.settings.amount || '0';
    scrollInput.style.cssText = 'width: 100px; height: 30px; background-color: #0d1117; color: #c9d1d9; border: 1px solid #30363d; border-radius: 3px; padding: 0 10px;';
    
    const directionContainer = document.createElement('div');
    directionContainer.style.cssText = 'display: flex; align-items: center; gap: 10px;';
    
    const directionLabel = document.createElement('span');
    directionLabel.textContent = 'Direction:';
    directionLabel.style.cssText = 'color: #c9d1d9;';
    ...
    dropdownButton.onclick = (event) => dialogDropdownManager.toggle(event, {
        items: ['Up', 'Down'],
        onSelect: (selectedText) => {
            actionItem.settings.direction = selectedText.toLowerCase();
        }
    });
    dropdownButton.style.cssText = 'width: 100px; height: 30px; text-align: left; padding-left: 10px; display: flex; justify-content: space-between; align-items: center; background-color: #0d1117;';
    dropdownButton.innerHTML = `
        <span>${actionItem.settings.direction || 'up'}</span>
        <span style="transform: rotate(90deg); margin-right: 5px; font-size: 18px;">&gt;</span>
    `;

    const dropdownMenu = document.createElement('ul');
    dropdownMenu.className = 'dropdown-menu custom-scrollbar';
    dropdownMenu.style.cssText = 'display: none; position: absolute; background-color: #0d1117; border: 1px solid #30363d; max-height: 200px; overflow-y: scroll; z-index: 2000; width: 100px; list-style-type: none; padding: 0; margin: 0; left: 0; top: 100%; transform: translateZ(0);';

    const scrollDelayContainer = document.createElement('div');
    scrollDelayContainer.style.cssText = 'display: flex; align-items: center; gap: 10px;';

    const scrollDelayLabel = document.createElement('span');
    scrollDelayLabel.textContent = 'Delay:';
    scrollDelayLabel.style.cssText = 'color: #c9d1d9;';

    const scrollDelayInput = document.createElement('input');
    scrollDelayInput.type = 'text';
    scrollDelayInput.id = 'scroll-delay';
    scrollDelayInput.value = actionItem.settings.delay || '0';
    scrollDelayInput.style.cssText = 'width: 100px; height: 30px; background-color: #0d1117; color: #c9d1d9; border: 1px solid #30363d; border-radius: 3px; padding: 0 10px;';

    dropdownContainer.appendChild(dropdownButton);
    dropdownContainer.appendChild(dropdownMenu);
    
    scrollAmountContainer.appendChild(scrollLabel);
    scrollAmountContainer.appendChild(scrollInput);
    
    directionContainer.appendChild(directionLabel);
    directionContainer.appendChild(dropdownContainer);

    scrollDelayContainer.appendChild(scrollDelayLabel);
    scrollDelayContainer.appendChild(scrollDelayInput);
    
    scrollContainer.appendChild(scrollAmountContainer);
    scrollContainer.appendChild(directionContainer);
    scrollContainer.appendChild(scrollDelayContainer);
...
case 'Sleep':
    ...
    sleepContainer.style.cssText = 'display: flex; align-items: center; gap: 10px;';

    const sleepLabel = document.createElement('span');
    sleepLabel.textContent = 'Sleep Duration (seconds):';
    sleepLabel.style.cssText = 'color: #c9d1d9;';

    const sleepInput = document.createElement('input');
    sleepInput.type = 'text';
    sleepInput.id = 'sleep-duration';
    sleepInput.value = actionItem.settings.duration || '0';
    sleepInput.style.cssText = 'width: 100px; height: 30px; background-color: #0d1117; color: #c9d1d9; border: 1px solid #30363d; border-radius: 3px; padding: 0 10px;';

    sleepContainer.appendChild(sleepLabel);
    sleepContainer.appendChild(sleepInput);
    content.appendChild(sleepContainer);
    break;

case 'Wait For Wave':
    const waveInput = document.createElement('input');
    waveInput.type = 'text';
    waveInput.id = 'wave-number';
    waveInput.value = actionItem.settings.wave || '0';
    waveInput.style.cssText = 'width: 100px; height: 30px; background-color: #0d1117; color: #c9d1d9; border: 1px solid #30363d; border-radius: 3px; padding: 0 10px;';
    
    const waveLabel = document.createElement('span');
    waveLabel.textContent = 'Wave Number:';
    waveLabel.style.cssText = 'color: #c9d1d9;';
    
    content.appendChild(waveLabel);
    content.appendChild(waveInput);
    break;

case 'Key Press':
    const keyContainer = document.createElement('div');
    keyContainer.style.cssText = 'display: flex; flex-direction: column; gap: 10px;';
    
    const keyInput = document.createElement('input');
    keyInput.type = 'text';
    keyInput.id = 'key-input';
    keyInput.readOnly = true;
    keyInput.value = actionItem.settings.key || 'None';
    ...
    const keyActionContainer = document.createElement('div');
    keyActionContainer.style.cssText = 'display: flex; align-items: center; gap: 10px;';
    
    const actionLabel = document.createElement('span');
    actionLabel.textContent = 'Action:';
    actionLabel.style.cssText = 'color: #c9d1d9;';
    
    const keyDropdownContainer = document.createElement('div');
    keyDropdownContainer.className = 'dropdown';
    keyDropdownContainer.style.cssText = 'position: relative;';

    const keyDropdownButton = document.createElement('button');
    keyDropdownButton.className = 'button';
    keyDropdownButton.type = 'button';
    keyDropdownButton.onclick = (event) => dialogDropdownManager.toggle(event, {
        items: ['Down', 'Up'],
        onSelect: (selectedText) => {
            actionItem.settings.action = selectedText.toLowerCase();
        }
    });
    keyDropdownButton.style.cssText = 'width: 100px; height: 30px; text-align: left; padding-left: 10px; display: flex; justify-content: space-between; align-items: center; background-color: #0d1117;';
    keyDropdownButton.innerHTML = `
        <span>${actionItem.settings.action || 'Down'}</span>
        <span style="transform: rotate(90deg); margin-right: 5px; font-size: 18px;">&gt;</span>
    `;

    const keyDropdownMenu = document.createElement('ul');
    keyDropdownMenu.className = 'dropdown-menu custom-scrollbar';
    keyDropdownMenu.style.cssText = 'display: none; position: absolute; background-color: #0d1117; border: 1px solid #30363d; max-height: 200px; overflow-y: scroll; z-index: 2000; width: 100px; list-style-type: none; padding: 0; margin: 0; left: 0; top: 100%; transform: translateZ(0);';

    keyDropdownContainer.appendChild(keyDropdownButton);
    keyDropdownContainer.appendChild(keyDropdownMenu);
    ...
    keyActionContainer.appendChild(actionLabel);
    keyActionContainer.appendChild(keyDropdownContainer);
    
    keyContainer.appendChild(keyInput);
    keyContainer.appendChild(keyActionContainer);
...
case 'Look':
    ...
    lookContainer.style.cssText = 'display: flex; flex-direction: column; gap: 10px;';
    
    const lookDirectionContainer = document.createElement('div');
    lookDirectionContainer.style.cssText = 'display: flex; align-items: center; gap: 10px;';
    
    const lookDirectionLabel = document.createElement('span');
    lookDirectionLabel.textContent = 'Direction:';
    lookDirectionLabel.style.cssText = 'color: #c9d1d9;';
    
    const lookDropdownContainer = document.createElement('div');
    lookDropdownContainer.className = 'dropdown';
    lookDropdownContainer.style.cssText = 'position: relative;';

    const lookDropdownButton = document.createElement('button');
    lookDropdownButton.className = 'button';
    lookDropdownButton.type = 'button';
    lookDropdownButton.onclick = (event) => dialogDropdownManager.toggle(event, {
        items: ['Up', 'Down'],
        onSelect: (selectedText) => {
            actionItem.settings.direction = selectedText.toLowerCase();
        }
    });
    lookDropdownButton.style.cssText = 'width: 100px; height: 30px; text-align: left; padding-left: 10px; display: flex; justify-content: space-between; align-items: center; background-color: #0d1117;';
    lookDropdownButton.innerHTML = `
        <span>${actionItem.settings.direction || 'up'}</span>
        <span style="transform: rotate(90deg); margin-right: 5px; font-size: 18px;">&gt;</span>
    `;

    const lookDropdownMenu = document.createElement('ul');
    lookDropdownMenu.className = 'dropdown-menu custom-scrollbar';
    lookDropdownMenu.style.cssText = 'display: none; position: absolute; background-color: #0d1117; border: 1px solid #30363d; max-height: 200px; overflow-y: scroll; z-index: 2000; width: 100px; list-style-type: none; padding: 0; margin: 0; left: 0; top: 100%; transform: translateZ(0);';

    lookDropdownContainer.appendChild(lookDropdownButton);
    lookDropdownContainer.appendChild(lookDropdownMenu);
    
    const lookDurationContainer = document.createElement('div');
    lookDurationContainer.style.cssText = 'display: flex; align-items: center; gap: 10px;';
    
    const lookDurationLabel = document.createElement('span');
    lookDurationLabel.textContent = 'Duration:';
    lookDurationLabel.style.cssText = 'color: #c9d1d9;';
    
    const lookDurationInput = document.createElement('input');
    lookDurationInput.type = 'text';
    lookDurationInput.id = 'look-duration';
    lookDurationInput.value = actionItem.settings.duration || '0';
    lookDurationInput.style.cssText = 'width: 100px; height: 30px; background-color: #0d1117; color: #c9d1d9; border: 1px solid #30363d; border-radius: 3px; padding: 0 10px;';
    
    lookDirectionContainer.appendChild(lookDirectionLabel);
    lookDirectionContainer.appendChild(lookDropdownContainer);
    lookDurationContainer.appendChild(lookDurationLabel);
    lookDurationContainer.appendChild(lookDurationInput);
    
    lookContainer.appendChild(lookDirectionContainer);
    lookContainer.appendChild(lookDurationContainer);
    
    content.appendChild(lookContainer);
    break;

case 'Key Hold':
    const keyHoldContainer = document.createElement('div');
    keyHoldContainer.style.cssText = 'display: flex; flex-direction: column; gap: 10px;';

    const keyHoldInput = document.createElement('input');
    keyHoldInput.type = 'text';
    keyHoldInput.id = 'key-hold-input';
    keyHoldInput.readOnly = true;
    keyHoldInput.value = actionItem.settings.key || 'None';
    keyHoldInput.style.cssText = 'width: 150px; height: 30px; background-color: #0d1117; color: #c9d1d9; border: 1px solid #30363d; border-radius: 3px; padding: 0 10px;';

    const holdDurationContainer = document.createElement('div');
    holdDurationContainer.style.cssText = 'display: flex; align-items: center; gap: 10px;';

    const holdDurationLabel = document.createElement('span');
    holdDurationLabel.textContent = 'Hold Duration:';
    holdDurationLabel.style.cssText = 'color: #c9d1d9;';

    const holdDurationInput = document.createElement('input');
    holdDurationInput.type = 'text';
    holdDurationInput.id = 'key-hold-duration';
    holdDurationInput.value = actionItem.settings.duration || '0';
    holdDurationInput.style.cssText = 'width: 100px; height: 30px; background-color: #0d1117; color: #c9d1d9; border: 1px solid #30363d; border-radius: 3px; padding: 0 10px;';

    keyHoldInput.onclick = function() {
        keyHoldInput.value = 'Press any key...';
        const handleKeyPress = function(e) {
            e.preventDefault();
            keyHoldInput.value = e.key.toUpperCase();
            document.removeEventListener('keydown', handleKeyPress);
        };
        document.addEventListener('keydown', handleKeyPress);
    };

    holdDurationContainer.appendChild(holdDurationLabel);
    holdDurationContainer.appendChild(holdDurationInput);

    keyHoldContainer.appendChild(keyHoldInput);
    keyHoldContainer.appendChild(holdDurationContainer);

    content.appendChild(keyHoldContainer);
    break;


            }

            createInteractiveDialog(data, content, [
                {
                    text: 'Save',
                    action: () => {
                        let newSettings = {};
...
case 'Move And Click':
    newSettings = {
        x: document.getElementById('click-x').value,
        y: document.getElementById('click-y').value,
        delay: document.getElementById('click-delay').value
    };
    break;
case 'Scroll':
    newSettings = {
        amount: document.getElementById('scroll-amount').value,
        direction: content.querySelector('.dropdown button span:first-child').textContent.toLowerCase(),
        delay: document.getElementById('scroll-delay').value
    };
    break;
case 'Sleep':
    newSettings = {
...
case 'Wait For Wave':
    newSettings = {
        wave: document.getElementById('wave-number').value
    };
    break;
case 'Key Press':
    newSettings = {
...
case 'Look':
    newSettings = {
        direction: content.querySelector('.dropdown button span:first-child').textContent.toLowerCase(),
        duration: document.getElementById('look-duration').value,
    };
    break;
case 'Custom OCR':
    newSettings = {
        text: document.getElementById('ocr-text').value,
        x1: document.getElementById('ocr-x1').value,
        y1: document.getElementById('ocr-y1').value,
        x2: document.getElementById('ocr-x2').value,
        y2: document.getElementById('ocr-y2').value
    };
    break;
case 'Key Hold':
    newSettings = {
        key: document.getElementById('key-hold-input').value,
        duration: document.getElementById('key-hold-duration').value,
    };
    break;
                        }

                        actionItem.settings = { ...actionItem.settings, ...newSettings };
        ...
        if (closestItem) {
            const isAfter = closestItem.offset > 0;
            actionsList.insertBefore(actionItem, isAfter ? closestItem.element.nextSibling : closestItem.element);
        } else {
...
function makeItemsDraggable() {
    const actionsList = document.getElementById('actionsList');
    const items = actionsList.getElementsByTagName('div');
        ...
        item.addEventListener('dragover', (e) => {
            ...
            if (draggingItem !== item) {
                if (offset < rect.height / 2) {
                    actionsList.insertBefore(draggingItem, item);
                } else {
                    actionsList.insertBefore(draggingItem, item.nextSibling);
                }
            }
        });
    });
...
function saveConfig() {
    ...
    // Get anti-afk data with saved coordinates
    const antiAfkData = {
    ...
    pywebview.api.load_config(selectedFile).then(data => {
        ...
        if (config.custom) {
            ...
            config.custom.actions.forEach(action => {
                const actionItem = document.createElement('div');
                actionItem.dataset.type = action.type;
                actionItem.settings = {...action.settings};
                ...
                actionItem.ondblclick = function() {
                    ...
                    createInteractiveDialog(action.type, content, [
                        {
                            text: 'Save',
                            action: () => {
                                actionItem.settings = {...action.settings};
...
class MacroBuilder:
    ...
    def get_dropped_items(self):
        return window.evaluate_js("""
            Array.from(document.querySelectorAll('#actionsList > div')).map(item => ({
                type: item.dataset.type,
                settings: item.settings
            }))
        """)
