...
function drag(ev) {
    // Add a flag to identify if this is a new item drag or reordering
...
function getDisplayText(type, settings) {
    switch(type) {
        case 'Move And Click':
            return `Move And Click (X: ${settings.x || '0'} Y: ${settings.y || '0'})`;
        case 'Scroll':
            return `Scroll (Amount: ${settings.amount || '0'} Direction: ${settings.direction || 'up'})`;
        case 'Sleep':
...
function loadConfig() {
    ...
    pywebview.api.load_config(selectedFile).then(data => {
        ...
        // Load anti-afk data with saved coordinates
...
if (config.custom) {
    ...
    config.custom.actions.forEach(action => {
        const fakeDropEvent = {
            ...
            dataTransfer: {
                getData: (type) => {
                    if (type === "text") return action.type;
                    if (type === "isNewItem") return "true";
                    return "";
                }
            }
        };
                
                // Call drop function with our fake event
                drop(fakeDropEvent);
                
                // Get the last added action item and update its settings
                const actionsList = document.getElementById('actionsList');
                const lastAction = actionsList.lastElementChild;
                lastAction.settings = action.settings;
                updateActionText(lastAction);
            });
        }
...
class MacroActions:
    ...
    def look(direction, duration):
        # Initialize pydirectinput with no pause
        ...
        # Lock position briefly to stabilize
        for _ in range(15):
            win32api.SetCursorPos((start_x, start_y))
            time.sleep(0.001)
    
        # Perform the timed movement
    ...
    def wait_for_wave(wave_number, api_instance):
        while not api_instance.stop_event.is_set():
            ...
            if current_wave and int(current_wave) >= int(wave_number):
                ...
                return True  # Return True to continue with next actions
            time.sleep(0.5)
    ...
    def custom_ocr(settings, api_instance):
        while not api_instance.stop_event.is_set():
            if api_instance.check_custom_ocr(settings):
                return True  # Text found, continue with next actions
            time.sleep(0.5)
    ...
    def run_macro_sequence(self):
        ...
        for item in items:
            ...
            if item['type'] == 'Move And Click':
                self.actions.move_and_click(int(item['settings']['x']), int(item['settings']['y']))
            
            elif item['type'] == 'Scroll':
                self.actions.scroll(item['settings']['amount'], item['settings']['direction'])
...
class Api:
    ...
    def replay_macro(self):
        ...
        if "Not Set" not in replay_click_status:
            ...
            # Verify replay was successful by checking if replay text is still present
            if self.check_for_replay():
                print("Replay click unsuccessful, retrying macro sequence")
                return True  # Return to main loop to try replay again
